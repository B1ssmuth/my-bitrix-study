...
abstract class AbstractIblockPropertyValuesTable extends DataManager
{
        ...
        foreach (static::getProperties() as $property) {
            if ($property['MULTIPLE'] === 'Y') {
                $map[$property['CODE'] . '_ELEMENT_NAME'] = new ExpressionField(
                    ...
                    sprintf(
                        '(SELECT GROUP_CONCAT(e.NAME SEPARATOR "\0") FROM %s as m JOIN b_iblock_element as e ON m.VALUE = e.ID WHERE m.IBLOCK_ELEMENT_ID = %%s AND m.IBLOCK_PROPERTY_ID = %d)',
    ...
    public static function getProperties(): array
    {
        ...
        $dbResult = PropertyTable::getList([
            'filter' => ['IBLOCK_ID' => static::IBLOCK_ID],
            'select' => ['ID', 'CODE', 'MULTIPLE', 'PROPERTY_TYPE']
        ]);
        
        static::$properties[static::IBLOCK_ID] = $dbResult->fetchAll();
        return static::$properties[static::IBLOCK_ID];
    }
